...
public sealed class PedidosCrudViewModel : CrudViewModelBase<Pedido, int>
{
    ...
    protected override string Describir(Pedido item) => item.Numero;
    ...
    private void Aplicar(Func<Pedido> transicion, Func<Pedido, string> aviso)
    {
        try
        {
...
/// <summary>
/// Procesos · Pedidos: los pedidos de clientes, antes de despacharlos.
///
/// Contenedor de un solo hijo, mismo patrón que <see cref="CuentasPorPagarViewModel"/>: lo que
/// hay que armar antes del listado no es una fuente de datos sino la cadena de servicios que
/// necesita "Despachar pedido" —que sí crea un despacho de Venta y, con él, su cuenta por cobrar
/// en Finanzas—, y eso no cabe en el constructor de un <see cref="PantallaCrudViewModel{T, TId}"/>.
///
/// El historial de despachos ya no vive aquí: es el submódulo <see cref="DespachosViewModel"/>.
/// Lo que sí se queda es el botón "Despachar pedido", que es donde se sabe qué falta entregar.
/// </summary>
public sealed class PedidosViewModel : PantallaViewModelBase
{
    ...
    public override void Recargar() => Pedidos.Recargar();
}
